...
export const CHAIN_CONFIG: Record<number, { routerAddress: string; wethAddress: string }> = {
  // Base mainnet
  8453: {
...
export interface SwapSimResult {
  canSwap:     boolean;
  buyTaxPct:   number;  // estimated buy tax (not split — total round-trip divided)
  sellTaxPct:  number;  // estimated sell tax
  totalLossPct: number; // combined round-trip loss %
  reason?:     string;
}

// 5-min cache per address+chainId
const _cache = new Map<string, { result: SwapSimResult; ts: number }>();
const CACHE_TTL = 5 * 60 * 1000;

export async function simulateEvmSwap(
  provider: ethers.JsonRpcProvider,
  targetTokenAddress: string,
  chainId: number = 8453,
  amountInWei: bigint = ethers.parseEther('0.001'),
...
async function _simulate(
  ...
  const router = new ethers.Contract(cfg.routerAddress, V2_ROUTER_ABI, provider);
  ...
  try {
    ...
    const tokensReceived: bigint = buyAmounts[1];
