...
exports.getTopNarrative = exports.recordNarrativeVolume = exports.recordHolderCount = void 0;
exports.analyzeToken = analyzeToken;
const holder_velocity_1 = require("./holder-velocity");
const dev_intelligence_1 = require("./dev-intelligence");
const slippage_1 = require("./slippage");
const narrative_flow_1 = require("./narrative-flow");
const cluster_1 = require("./cluster");
const scoring_1 = require("@lib/scoring");
const rug_1 = require("@lib/rug");
const gad_score_1 = require("@lib/gad-score");
const narrative_1 = require("@lib/narrative");
const social_1 = require("@lib/social");
const lifecycle_1 = require("@lib/lifecycle");
const survival_1 = require("@lib/survival");
const opportunity_1 = require("@lib/opportunity");
const memory_1 = require("@lib/memory");
var holder_velocity_2 = require("./holder-velocity");
Object.defineProperty(exports, "recordHolderCount", { enumerable: true, get: function () { return holder_velocity_2.recordHolderCount; } });
var narrative_flow_2 = require("./narrative-flow");
Object.defineProperty(exports, "recordNarrativeVolume", { enumerable: true, get: function () { return narrative_flow_2.recordNarrativeVolume; } });
Object.defineProperty(exports, "getTopNarrative", { enumerable: true, get: function () { return narrative_flow_2.getTopNarrative; } });
// ─── Main ─────────────────────────────────────────────────────────────────────
async function analyzeToken(input) {
    ...
    const marketRegime = input.marketRegime ?? 'SIDEWAYS';
    const regimeMultiplier = input.regimeMultiplier ?? 1.0;
    // ── 1. Core scoring ─────────────────────────────────────────────────────────
    const aiScore = (0, scoring_1.calculateAiScore)({
    ...
    // ── 2. Rug probability ────────────────────────────────────────────────────
    const rugResult = (0, rug_1.calculateRugProbability)({
    ...
    // ── 3. Narrative ──────────────────────────────────────────────────────────
    const narrativeTag = (0, narrative_1.detectNarrative)(input.symbol, input.name);
    const narrativeMomentum = narrative_1.DEFAULT_NARRATIVE_STRENGTH[narrativeTag] ?? 0;
    if (input.volume24h > 0)
        (0, narrative_flow_1.recordNarrativeVolume)(narrativeTag, input.volume24h);
    // ── 4. Hype / Social ───────────────────────────────────────────────────────
    const hypeResult = (0, social_1.calculateHypeScore)({
    ...
    // ── 5. Lifecycle ──────────────────────────────────────────────────────────
    const lifecycleResult = (0, lifecycle_1.detectLifecycle)({
        ageHours,
    ...
    // ── 6. Survival ───────────────────────────────────────────────────────────
    const survivalResult = (0, survival_1.calculateSurvivalScore)({
        ageHours,
        liquidityUsd: input.liquidityUsd,
    ...
    // ── 7. Whale / Smart Money ────────────────────────────────────────────────
    const whaleAccumulation = Math.min(100, Math.max(0, (input.whaleNetFlow ?? 0) > 0
    ...
    const convictionScore = Math.min(100, Math.round((Math.min(ageHours, 24) / 24) * 40 +
        Math.min(buySellRatio - 1, 2) * 20 +
        (whaleAccumulation > 50 ? 20 : 0) +
        (input.top10HolderPct < 40 ? 20 : 0)));
    // ── 8. Alpha Memory ───────────────────────────────────────────────────────
    const snapshot = (0, memory_1.buildTokenSnapshot)({
        aiScore,
    ...
    const memoryResult = (0, memory_1.calculateAlphaSimilarity)(snapshot, []);
    // ── 9. Opportunity Score ──────────────────────────────────────────────────
    const volBreakoutScore = input.volume5m > 0 && input.volume1h > 0
        ? Math.min(100, (input.volume5m / (input.volume1h / 12)) * 50) : 50;
    const opportunityResult = (0, opportunity_1.calculateOpportunityScore)({
        narrativeMomentum,
        whaleAccumulation,
    ...
    // ── 10. GAD Score ─────────────────────────────────────────────────────────
    const gadResult = (0, gad_score_1.calculateGadScore)({
        aiScore,
    ...
    // ── 11. Holder Velocity ───────────────────────────────────────────────────
    if (input.holders > 0)
        (0, holder_velocity_1.recordHolderCount)(input.mint, input.holders);
    const holderVelocity = (0, holder_velocity_1.calculateHolderVelocity)(input.mint, input.holders);
    // ── 12. Wallet Quality (heuristic) ────────────────────────────────────────
    const walletQuality = {
        qualityScore: Math.min(100, Math.round((input.top10HolderPct < 50 ? 40 : 10) +
    ...
    const slippageRisk = await (0, slippage_1.analyzeSlippage)(input.mint, input.liquidityUsd, input.priceUsd);
    const exitLiquidityScore = Math.min(100, Math.round((input.liquidityUsd / 10000) * 30 +
        (1 - slippageRisk.usd1000 / 100) * 40 +
        (buySellRatio > 0.5 ? 30 : 0)));
    // ── 14. Narrative Capital Flow ────────────────────────────────────────────
    const narrativeFlow = (0, narrative_flow_1.getNarrativeFlow)(narrativeTag, input.volume24h);
    // ── 15. Cluster Detection (async, best-effort for new tokens) ────────────
    let clusterDetection = { bundledWallets: 0, clusterRisk: 'NONE', coordinatedBuyPct: 0, patterns: [] };
    if (input.heliusApiKey && ageHours < 2) {
        clusterDetection = await (0, cluster_1.detectClusters)(input.mint, input.heliusApiKey).catch(() => clusterDetection);
    }
    // ── 16. Developer Intelligence ────────────────────────────────────────────
    const devIntelligence = input.devWallet && input.heliusApiKey
        ? await (0, dev_intelligence_1.analyzeDevBehavior)(input.mint, input.devWallet, input.heliusApiKey).catch(() => ({
            devWallet: input.devWallet, devHolding: true, devSoldPct: 0,
            devInitialBuySol: 0, devTransferCount: 0, devConviction: 'UNKNOWN', riskFlag: false,
        }))
        : { devWallet: '', devHolding: true, devSoldPct: 0, devInitialBuySol: 0, devTransferCount: 0, devConviction: 'UNKNOWN', riskFlag: false };
    // ── 17. Confidence ────────────────────────────────────────────────────────
    const dataPoints = [
