...
const slippageCache = new Map<string, { data: SlippageRisk; expires: number }>();

async function simulateJupiterSell(
  mint: string,
  amountUsd: number,
  priceUsd: number,
): Promise<number> {
  if (priceUsd <= 0) return 99;
  // Convert USD to token amount (assuming 6 decimals; will try both)
  const tokenAmount = Math.round((amountUsd / priceUsd) * 1e6);
  if (tokenAmount <= 0) return 0;

  try {
    const r = await axios.get(JUPITER_QUOTE, {
      params: {
        inputMint: mint,
    ...
    const data = r.data;
    if (!data?.outAmount || !data?.inAmount) return 50;
...
export async function analyzeSlippage(
  ...
  const result: SlippageRisk = { usd100, usd500, usd1000, usd5000, exitScore };
  slippageCache.set(mint, { data: result, expires: Date.now() + 120_000 });
  return result;
}
